pipeline {
   ...
   stages {
       stage ('display env and checkout code'){
           ...
           parallel (
               ...
               'CheckOutCode':{
                   git branch: 'dev', credentialsId: 'githubcred', url: 'https://github.com/Kammula280/javawebappp.git'
       ...
       stage('Build docker image'){
           steps{
       sh "docker build -t kammula280/javawebapp ."
           }
       }
       ...
       stage ('Push Docker Image to Repository'){
           steps{
               withCredentials([string(credentialsId: 'docker_hub_credentials', variable: 'docker_hub_credentials')]) {
                   ...
                   sh "docker push kammula280/javawebapp:latest"
                   }
       }
       }
   stage('Get Kubeconfig'){
       ...
       stage('Deploy to kuberentes'){
           steps{
               withAWS(credentials: 'kuberentes', region: 'ap-south-1') {
               sh "kubectl apply -f javawebapp.yml"
               }
           }
       }
   }
